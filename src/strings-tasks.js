...
function removeFirstOccurrences(str, value) {
  ...
  return (
    ...
    str.slice(firstIndex !== -1 ? firstIndex + value.length : firstIndex)
  );
}
...
function removeLastOccurrences(str, value) {
  ...
  return (
    ...
    str.slice(lastIndex !== -1 ? lastIndex + value.length : lastIndex)
  );
}
...
function sumOfCodes(str) {
  if (!str) return 0;
  return Array.from(str).reduce((acc, symbol) => acc + symbol.charCodeAt(0), 0);
}
...
function formatTime(minutes, seconds) {
  const minForm = minutes.toString().padStart(2, '0');
  const secForm = seconds.toString().padStart(2, '0');
...
function isPalindrome(str) {
  const strForm = str.toLowerCase().replace(/[^a-z]/g, '');
  return strForm.split('').reverse().join('') === strForm;
}
...
function findLongestWord(sentence) {
  ...
  const lenArr = sentenceArr.map((value) => value.length);
  const maxLen = Math.max(...lenArr);
  const index = lenArr.findIndex((l) => l === maxLen);
...
function encodeToRot13(str) {
  ...
  return strArr
    .map((symbol) => {
      ...
      const curValue = symbol.charCodeAt(0);
      const maxValue = /[a-z]/.test(symbol)
        ? 'z'.charCodeAt(0)
        : 'Z'.charCodeAt(0);
      const minValue = /[a-z]/.test(symbol)
        ? 'a'.charCodeAt(0)
        : 'A'.charCodeAt(0);
      return String.fromCharCode(
        curValue + 13 > maxValue
          ? minValue + 12 - (maxValue - curValue)
          : curValue + 13
      );
    })
    .join('');
...
function getCardId(value) {
  ...
  return symbol.indexOf(value.slice(-1)) * 13 + arr.indexOf(value.slice(0, -1));
}
